...
```bash
oj-problem-import batch --help
# 或
oj-problem-import batch -h
```

帮助中会列出支持的输入形式、并发/重试/输出目录参数，以及常用示例。
...
- `-w, --max-workers`: 最大并行工作进程数（默认: 4）。数值越高越快，但会占用更多 CPU、内存和 Docker 资源。
- `-m, --max-iterations`: 每个任务的最大迭代次数（默认: 20）。复杂题目可以适当调大。
- `-r, --max-retries`: 每个任务失败后的最大重试次数（默认: 2）。
- `-o, --output-dir`: 输出根目录（默认: outputs）。目录模式会保留原始目录结构。

### 运行前提示

batch 启动后会先提示支持的输入形式，并在扫描失败时给出可执行建议，例如：

```text
批量模式提示:
  输入支持: 单个文件、多个文件、逗号分隔列表、目录
  目录会自动扫描 .txt/.md/.markdown 文件，并保留原始目录结构
  查看完整用法: oj-problem-import batch --help
```
