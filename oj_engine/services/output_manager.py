...
class OutputManager:
    ...
    def save_result(self, state: Dict[str, Any], problem_title: str = "unnamed") -> Path:
        ...
        # 3. 保存测试数据 (成对的 .in/.out 文件)
        if state.get("test_cases"):
            test_data_dir = output_path / "tests"
            test_data_dir.mkdir(exist_ok=True)
            
            for test_case in state["test_cases"]:
                test_id = test_case.get('id', 1)
                
                # 保存 .in 文件
                in_file = test_data_dir / f"{test_id}.in"
                with open(in_file, 'w', encoding='utf-8') as f:
                    f.write(test_case.get('input', ''))
                
                # 保存 .out 文件
                out_file = test_data_dir / f"{test_id}.out"
                with open(out_file, 'w', encoding='utf-8') as f:
                    f.write(test_case.get('output', ''))
            
            print(f"  ✓ 保存测试数据: {len(state['test_cases'])} 组 (.in/.out)")
...
for i in ../tests/*.in; do
    echo "Testing $i..."
    python solution.py < "$i" > my_out.txt
    diff my_out.txt "${i%.in}.out"
done
```
...
```bash
cd codes
python generator.py > new_test.in
python solution.py < new_test.in > new_test.out
```
...
"""
        ...
        # 添加数据强度校验结果
        if state.get('test_data_stats'):
            stats = state['test_data_stats']
            content += f"""
## 测试数据统计

- **总数**: {stats.get('total', 0)} 组
- **小数据**: {stats.get('small', 0)} 组 (边界值、特殊情况)
- **中等数据**: {stats.get('medium', 0)} 组 (常规规模)
- **大数据**: {stats.get('large', 0)} 组 (极端情况、最坏情况)
- **AI 尝试次数**: {stats.get('attempts', 0)}
