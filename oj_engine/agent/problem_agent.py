...
1. 分析题面：提取题名、输入输出格式、约束、样例、隐含边界，以及“评测用例规模与约定”中的分档条件。
   - 题面约束优先于用户的分档偏好。用户提出的“2小5中3大”“更强数据”等要求，只能决定数量和强度倾向，不能放宽或覆盖题面合法性约束。
   - 先建立约束清单：区分部分分约束与 100% 约束，明确每个变量的上下界、结构性质、唯一性/连通性/非退化等条件。
   - 所有测试数据都必须满足题面 100% 约束；小数据优先覆盖部分分约束，大数据只能接近题面上限，不得超过上限或违反结构条件。
2. 确定标答语言和文件名：
   - 任务文件显式指定语言时，使用该语言。
...
5. 生成测试：
   ...
   - 先把题面约束写入 generator 中的常量、档位策略和校验函数；每组生成后必须自检，发现不满足约束就修正或重新生成。
   - 如果题面给出类似“20% 保证坐标 <= 100，100% 保证 n <= 500、坐标 <= 10000、线段不退化”的约定，大数据也必须满足 `n <= 500`、坐标不超过 10000、线段端点不相同；可以覆盖重合、共线、交点无限等题面允许的情况，但不能制造非法输入。
   - 需要图、树、DAG、排列、几何对象等结构时，先构建合法数据结构，再随机标签、权值、坐标或输出顺序。不要直接用随机值碰运气：树应先保证连通无环，连通图应先建生成树再加边，DAG 应先确定拓扑序，简单图应显式避免自环和重边。
   - 如果需要评估时空风险，可以自行修改或包装源代码插桩测量；沙箱工具默认不返回运行时间或内存占用。若用户没有要求极限强度，发现标答明显超时或资源风险过高时，应降低大数据强度后重新生成；如果用户明确要求“不要太贴心”“就要最极限情况”，则在题面合法范围内尽量贴近上界，并在总结中说明风险。
...
- 数据有效：生成的数据必须满足所有输入约束，覆盖最小值、最大值、重复值、随机常规、结构性极端情况；任何小/中/大分档都不得突破题面的 100% 范围、非退化条件或结构合法性。
...
4. 确保所有测试数据都能被标答正确处理
5. 确保测试数据强度分布符合要求: **30%小 + 50%中 + 20%大/边缘**。如果用户给出类似“2小5中3大”的分布，它只表示用例数量分配，不表示可以突破题面约束
6. 必须先抽取并遵守题面中的“评测用例规模与约定”:
   - 题面 100% 约束是所有用例的硬上限；大数据只能接近题面上限，不得超过
   - 部分分约束用于指导小数据覆盖，例如“20% 坐标 <= 100”应优先放入小数据，而不是让大数据违反“100% 坐标 <= 10000”
   - 非退化、唯一性、连通性、无自环/无重边、排列合法性等结构约束必须在生成器中显式保证
7. 生成结构化数据时要先构建合法结构，再随机化细节:
   - 树：先生成 n 个点和 n-1 条连通无环边，再打乱点编号或边顺序
   - 连通图：先生成一棵生成树，再按约束补边
   - DAG：先生成拓扑序，再只添加从前到后的边
   - 几何对象：先满足坐标范围和非退化条件，再构造共线、重合、相交、边界等强数据
8. 如需评估时空风险，可以自行修改或包装源代码插桩测量；沙箱工具默认不返回运行时间或内存占用。若用户没有要求极限强度，发现标答明显超时或资源风险过高时，应降低数据强度并重新验证；若用户明确要求“不要太贴心”“就要最极限情况”，则在题面合法范围内按用户说明贴近上界
